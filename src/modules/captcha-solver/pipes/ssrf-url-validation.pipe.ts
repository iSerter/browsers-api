 ...
 * 
 * Validates URL parameters to prevent SSRF attacks.
 * Can be used as a parameter-level pipe for URL validation.
 */
@Injectable()
export class SsrfUrlValidationPipe implements PipeTransform<string, Promise<string>> {
  ...
  async transform(value: string, metadata: ArgumentMetadata): Promise<string> {
    if (!value) {
      return value; // Let other validators handle empty values
    }
    ...
    try {
      ...
      this.logger.warn('SSRF attempt blocked via pipe', {
        url: value,
        reason: errorMessage,
      });
